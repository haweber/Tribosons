...
int ScanChain( TChain* chain, bool fast = true, int nEvents = -1, string skimFilePrefix = "test") {
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	if(i3l.size()>=3){
	  ...
	  if(nW==3&&(lep_mc_Id()[i3l[0] ]>0&&lep_mc_Id()[i3l[1] ]>0&&lep_mc_Id()[i3l[3] ]>0)) sn2 = "chargeflips";//W+W+W+ - it could be +++ final state, but at the end this final state will be vetoed, so if reco is ++- (e.g.), then this is a chargeflip
	  else if(nW==3&&(lep_mc_Id()[i3l[0] ]<0&&lep_mc_Id()[i3l[1] ]<0&&lep_mc_Id()[i3l[3] ]<0)) sn2 = "chargeflips";//W+W+W+ - it could be +++ final state, but at the end this final state will be vetoed, so if reco is ++- (e.g.), then this is a chargeflip
