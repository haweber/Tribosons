...
int ScanChain( TChain* chain, bool fast = true, int nEvents = -1, string skimFilePrefix = "test", int chainnumber=1) {
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	if(   jets_p4()[n].Pt()>=20&&fabs(   jets_p4()[n].Eta())<=3.0)                           ++njsoft20;
	...
	for(unsigned int j1 = 0; j1<jets_p4().size();++j1){
	  ...
	  for(unsigned int j2 = 0; j2<jets_p4().size();++j2){
	    if(j1==j2) continue;
	...
	MTs.push_back(mT(lep_p4()[i3l[i] ],MET));
     ...
     sort(MTs.begin(),MTs.end(),sortDecreasing);
      ...
      int CR3lpresel = -1;
      //SS
      SRSSpresel = isSRSS(iSS,      vSS,true ,MTmax,  njsoft20,nb,Mjj,MjjL,Detajj,MET,0,false,false,1);//enter variables for quicker calculation
	...
	int x;
	int SFOScounter = 0;
	if(numj<0||numb<0) getalljetnumbers(numj,x,numb, 0);
	if(numj <=1)           SFOScounter =  -5;
	if(numb!=0)     SFOScounter =  -5;
	if(i3l.size()!=3) SFOScounter =  -5;
	LorentzVector met = MET;
	float DPhilllMET = deltaPhi((lep_p4()[i3l[0] ]+lep_p4()[i3l[1] ]+lep_p4()[i3l[2] ]).Phi(),met.Phi());
	...
	bool pass0X(false), pass1X(false), pass2X(false);   //SR
	...
	if(SFOScounter==0){
	  ...
	  pass0X = pass0;
	  if(met.Pt()<30) pass0X = false;
	  if(fabs((lep_p4()[i3l[0] ]+lep_p4()[i3l[1] ]+lep_p4()[i3l[2] ]).M()-MZ)<10.) pass0X = false;
	}
	if(SFOScounter==1){
	  pass1 = true;
	  float metcut = 40.;
	  ...
	  pass1X = pass1;
	  if(thirdlep>=0&&mT(lep_p4()[thirdlep],met)<90.) pass1X = false;
	  if(pTlll<60.)  pass1X = false;
	  if(met.Pt()<metcut)  pass1X = false;
	  if(fabs((lep_p4()[i3l[0] ]+lep_p4()[i3l[1] ]+lep_p4()[i3l[2] ]).M()-MZ)<10.) pass1X = false;
	}
	if(SFOScounter==2){
	  pass2 = true;
	  ...
	  pass2X = pass2;
	  if(pTlll<60.) pass2X = false;
	  if(met.Pt()<55) pass2X = false;
	  if(fabs((lep_p4()[i3l[0] ]+lep_p4()[i3l[1] ]+lep_p4()[i3l[2] ]).M()-MZ)<10.) pass2X = false;
	}
	if(pass0 ) SR3lpresel =  0;
	if(pass1 ) SR3lpresel =  1;
	if(pass2 ) SR3lpresel =  2;
	if(DPhilllMET>=2.7&&pass0X) SR3l =  0;
	if(DPhilllMET>=2.5&&pass1X) SR3l =  1;
	if(DPhilllMET>=2.5&&pass2X) SR3l =  2;
      }
	...
	if(!isData()||!blindSR){
	  ...
	  for(unsigned int n = 0; n<jets_p4().size();++n){
	    if(!(   jets_p4()[n].Pt()>=20&&fabs(   jets_p4()[n].Eta())<=3.0)) continue;
	  ...
	  if(         passMjjL&&passDeta&&passMll&&passMET&&passMT){
	    ...
	    if(minDRlj<1.5&&met_pt()>40)                                fillhisto(histos, "SR_addone_dropMjj",     mysample, mysn,19.,      weight);//minDRlj
	    if(minDRlj<1.5&&Mjj<120.)                                   fillhisto(histos, "SR_addone_dropMjj",     mysample, mysn,20.,      weight);//minDRlj
	    if(minDRlj<1.5&&met_pt()>40&&Mjj<120.)                      fillhisto(histos, "SR_addone_dropMjj",     mysample, mysn,21.,      weight);//minDRlj
	  }
	  ...
	  if(SRSSpresel==0){
	    ...
	    if(passMjjL&&passMll&&passMET&&passMT) fillhisto(histos, "SR_additionaltests",     mysample, mysn, 8.,      weight);//drop Deta, Mjj
	    if(passMjjL&&passMll&&passMET&&passMT&&minDRlj<1.5) fillhisto(histos, "SR_additionaltests",     mysample, mysn, 9.,      weight);//drop Deta, Mjj, minDRlj<1.5
	    if(passMjjL&&passMll&&passMET&&passMT&&minDPhilj<1.5) fillhisto(histos, "SR_additionaltests",     mysample, mysn, 10.,      weight);//drop Deta, Mjj, minDPhilj<1.5
	  }
	  ...
	  if(SRSSpresel==2){
	    ...
	    if(passMjjL&&passMll&&passMET&&passMT) fillhisto(histos, "SR_additionaltests",     mysample, mysn, 4.,      weight);//drop Deta, Mjj
	    if(passMjjL&&passMll&&passMET&&passMT&&minDRlj<1.5) fillhisto(histos, "SR_additionaltests",     mysample, mysn, 5.,      weight);//drop Deta, Mjj, minDRlj<1.5
	    if(passMjjL&&passMll&&passMET&&passMT&&minDPhilj<1.5) fillhisto(histos, "SR_additionaltests",     mysample, mysn, 6.,      weight);//drop Deta, Mjj, minDPhilj<1.5
	  }
	}
      }
	...
	if(SR3lpresel==1||CR3lpresel==1){
	  if(passMlll&&passMSFOSorg&&passMT3rd){
	  ...
	  if(passMlll&&passMSFOS20&&passMT3rd){
	  ...
	  if(passMlll&&passMSFOS15&&passMT3rd){
  ...
  SaveHistosToFile("rootfiles/ImproveStuffv2NewBabe_diffjets.root",histos,true,true,(chainnumber==0));
