...
map<string, pair<vector<double>,vector<double> > > bookhistograms(map<string,vector< float> >& binning, string samplename, vector<string> histonames, vector<int> hbins, vector<float> hlow, vector<float> hup, TDirectory *rootdir, int splitWW=0);
...
bool  fillSRhisto(map<string, pair<vector<double>,vector<double> > > &histos, string histoname, string sample, string sn, string sn2, int SRSS, int SR3l, float weight, float weight3l=-2e6, bool fillsample=true);
bool  fillhisto(map<string, pair<vector<double>,vector<double> > > &histos, map<string,vector< float> > binning, string histoname, string sample, string sn, float value, float weight, bool fillsample=true);
bool  SaveHistosToFile(string filename, map<string, pair<vector<double>,vector<double> > > histos, map<string,vector< float> > binning, bool addunderflow=true, bool addoverflow=true, bool deletefile=false);
