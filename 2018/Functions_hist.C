...
map<string, pair<vector<double>,vector<double> > > bookhistograms(map<string,vector< float> >& binning, string samplename, vector<string> histonames, vector<int> hbins, vector<float> hlow, vector<float> hup, TDirectory *rootdir, int splitWW){
  map<string, pair<vector<double>,vector<double> > > histos;
  if(histonames.size()!=hbins.size()) return histos;
  if(histonames.size()!=hlow.size())  return histos;
  if(histonames.size()!=hup.size())   return histos;
  for(unsigned int i = 0; i<histonames.size(); ++i){
    string mapname = histonames[i];
    vector<float> bins;
    for(int j=0;j<hbins[i]+1;++j) bins.push_back(hlow[i]+(float)j*(hup[i]-hlow[i])/(float)hbins[i]); 
    const unsigned int nbins = hbins.size()+2;
    vector<double> yields, errors;
    yields.clear(); errors.clear();
    for(unsigned int j = 0; j<nbins; ++j){ yields.push_back(0); errors.push_back(0); }
    pair<vector<double>,vector<double> > mypair = make_pair(yields,errors);
    mapname = histonames[i] + "_"+samplename;
    if(histos.count(mapname) == 0 ) { histos[mapname] = mypair; binning[mapname] = bins; }
...
bool deleteHistograms(map<string, pair<vector<double>,vector<double> > > histos){
  //for(map<string,TH1D*>::iterator h=histos.begin(); h!=histos.end();++h) delete h->second;
  return true;
}

bool fillSRhisto(map<string, pair<vector<double>,vector<double> > > &histos, string histoname, string sample, string sn, string sn2, int SRSS, int SR3l, float weight, float weight3l, bool fillsample){
  if(SRSS<0&&SR3l<0) return true;
  float weight2 = weight3l;
  if(weight3l<-1e6) weight2 = weight;
  //cout << __LINE__ << " " << histoname+"_"+sample << " " << histos.count(histoname+"_"+sample) << " " << histos.count(histoname+"_"+sn) << " " << histos.count(histoname+"_"+sn2) << " " << weight << " " << SRSS << " " << SR3l << endl;
  //for(unsigned int i = 0; i<=7; ++i) cout << i << " " << histos[histoname+"_"+sample].first[i] << " " << histos[histoname+"_"+sample].second[i] << endl;
  if(fillsample){
    if(SRSS==0) {  histos[histoname+"_"+sample].first[1] = histos[histoname+"_"+sample].first[1] + weight;  histos[histoname+"_"+sample].second[1] = sqrt(pow(histos[histoname+"_"+sample].second[1],2)+pow(weight ,2)); }
    if(SRSS==1) {  histos[histoname+"_"+sample].first[2] = histos[histoname+"_"+sample].first[2] + weight;  histos[histoname+"_"+sample].second[2] = sqrt(pow(histos[histoname+"_"+sample].second[2],2)+pow(weight ,2)); }
    if(SRSS==2) {  histos[histoname+"_"+sample].first[3] = histos[histoname+"_"+sample].first[3] + weight;  histos[histoname+"_"+sample].second[3] = sqrt(pow(histos[histoname+"_"+sample].second[3],2)+pow(weight ,2)); }
    if(SR3l==0) {  histos[histoname+"_"+sample].first[4] = histos[histoname+"_"+sample].first[4] + weight2; histos[histoname+"_"+sample].second[4] = sqrt(pow(histos[histoname+"_"+sample].second[4],2)+pow(weight2,2)); }
    if(SR3l==1) {  histos[histoname+"_"+sample].first[5] = histos[histoname+"_"+sample].first[5] + weight2; histos[histoname+"_"+sample].second[5] = sqrt(pow(histos[histoname+"_"+sample].second[5],2)+pow(weight2,2)); }
    if(SR3l==2) {  histos[histoname+"_"+sample].first[6] = histos[histoname+"_"+sample].first[6] + weight2; histos[histoname+"_"+sample].second[6] = sqrt(pow(histos[histoname+"_"+sample].second[6],2)+pow(weight2,2)); }
  }
  if(sample!=sn){
    if(SRSS==0) {  histos[histoname+"_"+sn    ].first[1] = histos[histoname+"_"+sn    ].first[1] + weight;  histos[histoname+"_"+sn    ].second[1] = sqrt(pow(histos[histoname+"_"+sn    ].second[1],2)+pow(weight ,2)); }
    if(SRSS==1) {  histos[histoname+"_"+sn    ].first[2] = histos[histoname+"_"+sn    ].first[2] + weight;  histos[histoname+"_"+sn    ].second[2] = sqrt(pow(histos[histoname+"_"+sn    ].second[2],2)+pow(weight ,2)); }
    if(SRSS==2) {  histos[histoname+"_"+sn    ].first[3] = histos[histoname+"_"+sn    ].first[3] + weight;  histos[histoname+"_"+sn    ].second[3] = sqrt(pow(histos[histoname+"_"+sn    ].second[3],2)+pow(weight ,2)); }
  }
  if(sample!=sn2){
    if(SR3l==0) {  histos[histoname+"_"+sn2   ].first[4] = histos[histoname+"_"+sn2   ].first[4] + weight2; histos[histoname+"_"+sn2   ].second[4] = sqrt(pow(histos[histoname+"_"+sn2   ].second[4],2)+pow(weight2,2)); }
    if(SR3l==1) {  histos[histoname+"_"+sn2   ].first[5] = histos[histoname+"_"+sn2   ].first[5] + weight2; histos[histoname+"_"+sn2   ].second[5] = sqrt(pow(histos[histoname+"_"+sn2   ].second[5],2)+pow(weight2,2)); }
    if(SR3l==2) {  histos[histoname+"_"+sn2   ].first[6] = histos[histoname+"_"+sn2   ].first[6] + weight2; histos[histoname+"_"+sn2   ].second[6] = sqrt(pow(histos[histoname+"_"+sn2   ].second[6],2)+pow(weight2,2)); }
  }
  //cout << __LINE__ << " " << histoname+"_"+sample << " " << histos.count(histoname+"_"+sample) << endl;
  //for(unsigned int i = 0; i<=7; ++i) cout << i << " " << histos[histoname+"_"+sample].first[i] << " " << histos[histoname+"_"+sample].second[i] << endl;
  return true;
}

bool fillhisto(map<string, pair<vector<double>,vector<double> > > &histos, map<string,vector< float> > binning, string histoname, string sample, string sn, float value, float weight, bool fillsample){
  //cout << __LINE__ << " " <<  histoname+"_"+sample << " " << binning.count(histoname+"_"+sample) << endl;
  //cout << binning[ histoname+"_"+sample].size() << endl;
  unsigned int bin = binning[ histoname+"_"+sample].size();//overflow bin;
  for(unsigned int i = 0; i<binning[ histoname+"_"+sample].size(); ++i){
    if(value<binning[ histoname+"_"+sample][i]) {
      bin = i;
      //cout << __LINE__ << ": " <<  histoname+"_"+sample << " " << binning[ histoname+"_"+sample].size() << " " << endl;
      //cout << __LINE__ << ": " <<  value << " " << binning[ histoname+"_"+sample][i] << " " << bin << endl;
      break;
    }
  }
  if(fillsample){  histos[histoname+"_"+sample].first[bin] = histos[histoname+"_"+sample].first[bin] + weight;  histos[histoname+"_"+sample].second[bin] = sqrt(pow(histos[histoname+"_"+sample].second[bin],2)+pow(weight ,2)); }
  if(sample!=sn){  histos[histoname+"_"+sn    ].first[bin] = histos[histoname+"_"+sn    ].first[bin] + weight;  histos[histoname+"_"+sn    ].second[bin] = sqrt(pow(histos[histoname+"_"+sn    ].second[bin],2)+pow(weight ,2)); }
  return true;
}

bool SaveHistosToFile(string filename, map<string, pair<vector<double>,vector<double> > > histos, map<string,vector< float> > binning, bool addunderflow, bool addoverflow, bool deletefile){
  map<string, TH1D*> hist;
  
  for(map<string, pair<vector<double>,vector<double> > >::iterator h=histos.begin(); h!=histos.end();++h){
    //pair<double*,double*> hh = h->second;
    //cout << __LINE__ << " " << h->first.c_str() << " " << binning[h->first].size() << " " << binning[h->first][0] << " " << binning[h->first][binning[h->first].size()-1] << endl;
    hist[h->first] = new TH1D(h->first.c_str(),"",binning[h->first].size()-1, binning[h->first][0], binning[h->first][binning[h->first].size()-1]);
    //add overflow
    if(addoverflow){
      h->second.first[binning[h->first].size()]  = h->second.first[0]+h->second.first[binning[h->first].size()+1];
      h->second.second[binning[h->first].size()] = sqrt(pow(h->second.second[binning[h->first].size()],2)+pow(h->second.second[binning[h->first].size()+1],2));
    }
    //add underflow
    if(addunderflow){
      h->second.first[1]  = h->second.first[0]+h->second.first[1];
      h->second.second[1] = sqrt(pow(h->second.second[0],2)+pow(h->second.second[1],2));
    }
    for(int i = 0; i<=hist[h->first]->GetNbinsX()+1; ++i){
      hist[h->first]->SetBinContent(i,h->second.first[ i]);
      hist[h->first]->SetBinError(  i,h->second.second[i]);
    }
  }
  ...
  for(map<string, pair<vector<double>,vector<double> > >::iterator h=histos.begin(); h!=histos.end();++h){
    if(f->GetListOfKeys()->Contains(h->first.c_str())){
      hist[h->first]->SetName((h->first+"new").c_str());
      TH1D *hold = (TH1D*)f->Get(h->first.c_str());
      hold->SetName((h->first+"old").c_str());
      TH1D *hnew = (TH1D*)hold->Clone(h->first.c_str());
      hnew->Add(hist[h->first]);
      hnew->Write(h->first.c_str(),TObject::kOverwrite);
    }
    else
      hist[h->first]->Write(h->first.c_str(),TObject::kOverwrite);
  }
  f->Close();
  cout << "Saved histos in " << f->GetName() << endl;
  delete f;
  for(map<string,TH1D*>::iterator h=hist.begin(); h!=hist.end();++h) delete h->second;
  return true;
}
